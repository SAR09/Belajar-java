...
import programmermuda.jpa.entity.Product;
import programmermuda.jpa.entity.SimpleBrand;
...
public class CriteriaTest {
    ...
    @Test
    void criteriaQueryWhereClauseUsingOperator() {
        EntityManagerFactory entityManagerFactory = JpaUtil.getEntityManagerFactory();
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        EntityTransaction entityTransaction = entityManager.getTransaction();
        entityTransaction.begin();
    ...
    void criteriaAgregratQuery() {
        ...
        // select b.id, min(p.price), max(p.price), avg(p.price) from Product p join p.brand b group by b.id having min(p.price) > 50.000
